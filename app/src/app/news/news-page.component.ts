...
export class NewsPageComponent {
      ...
      .subscribe((x) => {
        const newArticles = x.articles.filter((article) => {
          return !this.news?.find(
            (found) =>
              found.dataSourceIdentifier == article.dataSourceIdentifier
          );
        });
  ...
  searchChange() {
    const foundTeam = this.teamList.find((team) => {
      return team.name
        .toLowerCase()
        .includes(this.searchTeamNews.toLowerCase());
    });
  ...
  filteredChange(filter: any) {
    ...
    this.filteredTeamList = this.teamList.filter((team) => {
      return team.name
        .toLocaleLowerCase()
        .includes(filter.target.value.toLocaleLowerCase());
    });
  }
